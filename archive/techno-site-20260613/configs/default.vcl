...
# Backend: Apache on port 81 (Varnish will handle port 80)
backend default {
    .host = "127.0.0.1";
    .port = "81";
    ...
    .probe = {
        .request =
            "GET /pub/static/version123/frontend/Magento/luma/en_US/images/logo.svg HTTP/1.1"
...
sub vcl_recv {
    ...
    # Ajax responses are dynamic and should not be cached
    if (req.url ~ "(\?|&)ajax=1") {
        return (pass);
    }
...
sub vcl_hash {
    ...
    if (req.url ~ "\?") {
        # CRITICAL: Do NOT strip filter parameters - they change the product collection!
        # Only strip sort/order params that don't change which products are shown
        # Filter params like color, price, brand, attributes MUST be preserved in cache hash
        set req.url = regsuball(req.url, "(&|\?)(product_list_order|product_list_dir|product_list_mode|a_la_une)=[^&]*", "");
        ...
        # Strip AJAX params (but Ajax requests should already be bypassed in vcl_recv)
        set req.url = regsuball(req.url, "(&|\?)(isAjax|format)=[^&]*", "");
    ...
    # Add device type to cache hash so mobile/desktop/tablet get different cached pages
    if (req.http.user-agent ~ "(?i)(iPhone|iPod|Android.*Mobile|Windows Phone|BlackBerry|Opera Mini|IEMobile)") {
        hash_data("mobile");
    } elsif (req.http.user-agent ~ "(?i)(iPad|Android(?!.*Mobile)|Silk)") {
        hash_data("tablet");
    } else {
        hash_data("desktop");
    }
...
sub vcl_backend_response {
    # Handle backend redirects (301/302) - pass them through to client
    # instead of trying to follow them which creates loops through Cloudflare
