...
MPTABLERATE_ACTIVE=$(mysql --defaults-extra-file=/home/dev/.my.cnf -se "SELECT value FROM core_config_data WHERE path = 'carriers/mptablerate/active';" 2>/dev/null)
...
METHODS_COUNT=$(mysql --defaults-extra-file=/home/dev/.my.cnf -se "SELECT COUNT(*) FROM mageplaza_tablerate_method WHERE status = 1;" 2>/dev/null)
...
GIFTCARD_POSITION=$(mysql --defaults-extra-file=/home/dev/.my.cnf -se "SELECT value FROM core_config_data WHERE path = 'amgiftcard/gift_card_account/checkout_position';" 2>/dev/null)
...
# Test currency
CURRENCY=$(mysql --defaults-extra-file=/home/dev/.my.cnf -se "SELECT value FROM core_config_data WHERE path = 'currency/options/base';" 2>/dev/null)
