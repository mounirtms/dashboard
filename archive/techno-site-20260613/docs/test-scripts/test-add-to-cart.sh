...
SHIPPING_COUNT=$(mysql --defaults-extra-file=/home/dev/.my.cnf -se "SELECT COUNT(*) FROM mageplaza_tablerate_method WHERE status = 1;" 2>/dev/null)
...
if [ ! -z "$SHIPPING_COUNT" ] && [ "$SHIPPING_COUNT" -gt 0 ]; then
    ...
    mysql --defaults-extra-file=/home/dev/.my.cnf -se "SELECT method_id, name FROM mageplaza_tablerate_method WHERE status = 1 LIMIT 5;" 2>/dev/null | column -t
else
