...
foreach ($answers as $answer) {
    ...
    if ($data) {
        $processedAnswer = [
            ...
            'rules' => isset($data['checkbox-rules']['value']) ? implode(', ', $data['checkbox-rules']['value']) : '',
            // Add rating field (default to 0 if not set)
            'rating' => 0
        ];
...
// Simple caching mechanism using file cache
$cacheFile = __DIR__ . '/cache/data_cache.json';
...
if (file_exists($cacheFile) && (time() - filemtime($cacheFile)) < $cacheTime) {
    ...
    if ($cachedData) {
        $processedAnswers = $cachedData;
    }
} else {
    // Save to cache
    file_put_contents($cacheFile, json_encode($processedAnswers));
}
...
// Initialize ratings with sample data for demonstration
...
foreach ($processedAnswers as $index => $answer) {
    ...
    $rating = rand(1, 5);
    $processedAnswers[$index]['rating'] = $rating;
    $ratingStats[$rating]++;
}
...
<body>
            ...
            <div class="card" 
                ...
                <div class="card-image">
                    <?php if (!empty($answer['photo'])): ?>
                        ...
                        <?php else: ?>
                            <img src="https://technostationery.com/pub/media/amasty/amcustomform/<?php echo htmlspecialchars($answer['photo']); ?>" alt="<?php echo htmlspecialchars($answer['title']); ?>">
                        <?php endif; ?>
                    <?php else: ?>
                        <img src="https://via.placeholder.com/400x250/4a4a4a/ffffff?text=Pas+d'image" alt="Pas d'image">
                    <?php endif; ?>
                    <div class="card-rating">
                        <div class="card-rating-value"><?php echo number_format($answer['rating'], 1); ?></div>
                ...
                <div class="card-content">
                    ...
                    <div class="action-buttons">
                        <button class="btn btn-rate" onclick="rateEntry(<?php echo $answer['id']; ?>)">
    ...
    <script>
        ...
        document.getElementById('sortBy').addEventListener('change', function() {
            ...
            cards.sort((a, b) => {
                switch(sortBy) {
                    ...
                    case 'rating_low':
                        const ratingC = parseInt(a.getAttribute('data-rating'));
                        const ratingD = parseInt(b.getAttribute('data-rating'));
        ...
        // Rating function
        function rateEntry(id) {
            const rating = prompt('Entrez une note entre 1 et 5 étoiles:', '5');
            if (rating !== null && rating >= 1 && rating <= 5) {
                // In a real implementation, this would send an AJAX request to update the rating
                alert('Note de ' + rating + ' étoiles enregistrée pour l\'entrée #' + id + '.');
        ...
        function deleteEntry(id) {
            if (confirm('Êtes-vous sûr de vouloir supprimer cette entrée ?')) {
                // In a real implementation, this would send an AJAX request to delete the entry
                alert('Entrée #' + id + ' supprimée avec succès.');
