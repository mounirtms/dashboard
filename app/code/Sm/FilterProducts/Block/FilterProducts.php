...
class FilterProducts extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    private function _featuredProducts(){
        ...
        $cacheKey = 'sm_filter_featured_' . md5(json_encode([$product_order_by, $product_order_dir, $count, $category_id, $this->_storeId]));
        ...
        $collection->getSelect()->distinct(true)->group('e.entity_id')->limit($count);
        $cache->save(serialize($collection), $cacheKey, ['sm_filter_products'], 3600); // 1 hour TTL
        return $collection;
    }
    ...
    private function _bestSellers(){
        ...
        !is_array($category_id) && $category_id = preg_split('/[\s|,|;]/', $category_id, -1, PREG_SPLIT_NO_EMPTY);
        $cache = $this->_objectManager->get('Magento\\Framework\\App\\CacheInterface');
        $cacheKey = 'sm_filter_bestsellers_' . md5(json_encode([$count, $category_id, $this->_storeId]));
        $cacheData = $cache->load($cacheKey);
        if ($cacheData !== false) {
            return unserialize($cacheData);
        }
        ...
        $collection->getSelect()
            ...
            ->limit($count);
        $cache->save(serialize($collection), $cacheKey, ['sm_filter_products'], 3600);
        return $collection;
    }
    ...
    private function _viewedProducts(){
        ...
        !is_array($category_id) && $category_id = preg_split('/[\s|,|;]/', $category_id, -1, PREG_SPLIT_NO_EMPTY);
        $cache = $this->_objectManager->get('Magento\\Framework\\App\\CacheInterface');
        $cacheKey = 'sm_filter_viewed_' . md5(json_encode([$count, $category_id, $this->_storeId]));
        $cacheData = $cache->load($cacheKey);
        if ($cacheData !== false) {
            return unserialize($cacheData);
        }
