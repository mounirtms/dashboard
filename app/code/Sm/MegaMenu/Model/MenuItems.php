...
class MenuItems extends AbstractModel implements MenuItemsInterface
{
	...
	public function getChildsDirectlyByItem($parent, $mode = 1)
	{
		...
		$cacheKey = 'megamenu_childs_' . md5($tableName . '_' . $depth_child_directly . '_' . $parent_id . '_' . $groupId);
		...
		try {
			...
			$this->cache->save(serialize($childrenItems), $cacheKey, ['megamenu'], 3600);
	...
	{
		$tableName = $this->_tableName;
		$cacheKey = 'megamenu_idparent_' . md5($tableName . '_' . $groupId . '_' . $levelStart . '_' . $statusChild);
		$cacheData = $this->cache->load($cacheKey);
		if ($cacheData !== false) {
			return unserialize($cacheData);
		}
	...
	public function getAllItemsInEqLv($item, $mode = 1, $attributes = '')
	{
		...
		$cacheKey = 'megamenu_eqlevel_' . md5($tableName . '_' . $depth_child_directly . '_' . $parent_id . '_' . $groupId . '_' . $mode . '_' . $attributes);
		$cacheData = $this->cache->load($cacheKey);
		if ($cacheData !== false) {
			return unserialize($cacheData);
		}
