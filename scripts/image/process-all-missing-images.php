<?php
/**
 * Process All Missing Images Script
 * 
 * This script processes the CSV file and organizes ALL images according to the requirements:
...
echo "📊 Found " . count($csvData) . " entries in CSV\n";
...
foreach ($csvData as $index => $entry) {
    ...
    if ($index % 50 == 0) {
        echo "🔄 Processing entry " . ($index + 1) . " of $totalEntries\n";
    }
    ...
    foreach ($matchingImages as $imgIndex => $sourceImagePath) {
        // Determine the final image name
        $finalImageName = $imageName;
        if ($imgIndex > 0) {
            $finalImageName .= '_' . ($imgIndex + 1);
        }
    ...
    switch ($imageInfo['mime']) {
        ...
        case 'image/webp':
            if (function_exists('imagecreatefromwebp')) {
                ...
                echo "⚠️ WebP support not available, skipping resize for: " . basename($imagePath) . "\n";
                return false;
            }
            break;
        default:
    ...
    switch ($imageInfo['mime']) {
        ...
        case 'image/webp':
            if (function_exists('imagewebp')) {
                $result = imagewebp($resizedImage, $imagePath, 85);
            }
            break;
    }
