...
LOG_FILE="${PROJECT_ROOT}/var/log/weekly-idrive-backup.log"
...
check_day() {
    if [ "$DAY_OF_WEEK" -ne 4 ]; then
        log "Today is not Thursday (day $DAY_OF_WEEK). This script is designed to run only on Thursdays."
        log "Exiting without performing backup."
        exit 0
    fi
    log "Today is Thursday. Proceeding with weekly backup."
}
...
# === Main function ===
main() {
    log "Starting weekly iDrive backup process..."
    ...
    success "Weekly iDrive backup completed successfully in ${DURATION} seconds!"
