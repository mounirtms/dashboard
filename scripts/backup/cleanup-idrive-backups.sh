...
# Cleanup old iDrive backups
# This script removes backups older than 30 days (4 weeks) to save space
# Designed to work with weekly backups

set -e

# === Configuration ===
PROJECT_ROOT="/home/technadminy7/public_html"
LOG_FILE="${PROJECT_ROOT}/var/log/cleanup-idrive-backups.log"
...
list_backups() {
    ...
    BACKUP_LIST=$(aws s3 ls "$S3_BUCKET/" --endpoint-url "$S3_ENDPOINT" | grep "PRE" | awk '{print $2}' | sed 's/\/$//')
...
identify_old_backups() {
    log "Identifying backups older than 30 days..."
    
    # Calculate cutoff date (keep last 4 weeks for weekly backups)
...
# === Main function ===
main() {
    log "Starting iDrive backup cleanup process..."
    ...
    if ! identify_old_backups; then
        ...
        success "Cleanup process completed (no deletions needed)"
        exit 0
    fi
    
    # Confirm before deleting
    OLD_COUNT=$(wc -l < /tmp/old_backups.txt)
    log "Ready to delete $OLD_COUNT old backups"
    ...
    success "iDrive backup cleanup completed successfully!"
