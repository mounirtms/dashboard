...
export default function SystemHealthPage() {
  ...
  const fetcher = useCallback(async (): Promise<SystemHealthBundle> => {
    const [ssh, services, network, csf] = await Promise.all([
      fetchSshConnections(),
      fetchServices(),
      fetchNetworkConnections(),
      fetchCsfFirewall(),
    ]);
    return { ssh, services, network, csf };
  }, []);

  const { data, loading, refreshing, error, refetch: loadData } = usePolling<SystemHealthBundle>(fetcher, 10_000);

  const sshData      = data?.ssh      ?? null;
  const servicesData = data?.services ?? null;
  const networkData  = data?.network  ?? null;
  const csfData      = data?.csf      ?? null;

  if (loading) {
    return (
      <Box sx={{ p: 2 }}>
        <Skeleton height={50} sx={{ mb: 2 }} />
        <Skeleton height={40} width={400} sx={{ mb: 3 }} />
        {[...Array(4)].map((_, i) => <Skeleton key={i} height={80} sx={{ mb: 2 }} />)}
      </Box>
    );
  }
    ...
    confirmAction(
      ...
      async () => {
        try {
          ...
          showSnackbar(getErrMsg(e) || 'Failed to kill session', 'error');
        } finally {
    ...
    confirmAction(
      ...
      async () => {
        try {
          ...
          showSnackbar(getErrMsg(e) || 'Failed to kill sessions', 'error');
        } finally {
          setActionLoading(false);
        }
      }
    ...
    confirmAction(
      ...
      async () => {
        try {
          ...
          showSnackbar(getErrMsg(e) || 'Failed to restart SSH', 'error');
        } finally {
          setActionLoading(false);
        }
      }
    ...
    confirmAction(
      ...
      async () => {
        try {
          ...
          showSnackbar(getErrMsg(e) || 'Action failed', 'error');
        } finally {
          setActionLoading(false);
        }
      }
                ...
                <Grid container spacing={1}>
                  {services.map((svc) => (
                    <Grid size={{ xs: 12, sm: 6, md: 4 }} key={svc.name}>
                      ...
                      >
                        <StatusBadge label={svc.status} color={statusColor(svc.status)} />
                        <Box sx={{ flex: 1 }}>
