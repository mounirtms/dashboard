    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          ...
          <Button variant="outlined" startIcon={<Refresh />} onClick={loadHistory} disabled={loading}>
            Refresh
          </Button>
        </Box>
      </Box>
      ...
      <TableContainer component={Paper} sx={{ bgcolor: 'transparent', border: '1px solid rgba(255,255,255,0.06)', flexGrow: 1, overflow: 'auto' }}>
        <Table stickyHeader size="small">
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={2} sx={{ py: 8 }}><LoadingState message="Fetching history..." /></TableCell>
              </TableRow>
