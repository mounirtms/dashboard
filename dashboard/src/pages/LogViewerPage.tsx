import { Box, Typography, Card, CardContent, Select, MenuItem, FormControl, InputLabel, Button, IconButton, Tooltip, TextField, Snackbar, Alert, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, Paper, Skeleton } from '@mui/material';
...
export default function LogViewerPage() {
  ...
  const fetchLogs = useCallback(() => {
    ...
    // Stale-while-revalidate: only show full loading spinner on first fetch
    if (!logData) {
      setLoading(true);
    } else {
      setRefreshing(true);
    ...
    if (type === 'bash') {
      fetchBashHistory(bashUser, Math.min(lines, 500))
        ...
        .finally(() => { setLoading(false); setRefreshing(false); fetchRef.current = false; });
      return;
    }
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Button variant="outlined" startIcon={<Refresh />} onClick={fetchLogs} disabled={loading || refreshing}>
            Refresh
          </Button>
      ...
      <Box sx={{ flexGrow: 1, minHeight: 0 }}>
        {error ? (
          ...
          <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 1, p: 2 }}>
            {[...Array(8)].map((_, i) => <Skeleton key={i} variant="rounded" height={40} />)}
          </Box>
        ) : logData?.structured ? (
