...
import { Box, Typography, Button, TextField, Chip, Dialog, DialogTitle, DialogContent, DialogActions, Select, MenuItem, FormControl, InputLabel, IconButton, Tooltip, Alert, Snackbar, Drawer, Grid, Card, CardContent, InputAdornment, Divider, Skeleton } from '@mui/material';
...
export default function MagentoProductsPage() {
  ...
  const handleUploadSubmit = async () => {
    ...
    for (const file of uploadFiles) {
      ...
      try {
        ...
        setToast({ message: `Upload failed for ${file.name}: ${getErrMsg(e)}`, severity: 'error' });
      }
    }
    setUploading(false);
    setUploadOpen(false);
  ...
  if (error && !products.length) return <Alert severity="error" sx={{ mb: 2 }}>Products error: {error}</Alert>;
      ...
      <DataGrid
        ...
        onRowSelectionModelChange={(model) => {
          const ids = model && typeof model === 'object' && 'ids' in model
            ? (Array.from((model as { ids: Set<number> }).ids) as number[])
            : [];
          setSelection(ids);
        }}
        rowSelectionModel={selection as unknown as import('@mui/x-data-grid').GridRowSelectionModel}
