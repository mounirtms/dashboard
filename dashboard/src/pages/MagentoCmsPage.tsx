...
import { Box, Typography, Button, Chip, Drawer, IconButton, Tooltip, Snackbar, Alert, Tabs, Tab, TextField, Dialog, DialogTitle, DialogContent, DialogActions, Divider, List, ListItemButton, ListItemText, Collapse, FormControlLabel, Switch, FormControl, Select, MenuItem, Skeleton } from '@mui/material';
...
export default function MagentoCmsPage() {
  ...
  const loadPages = useCallback(async () => {
    try {
      ...
      setToast({ message: getErrMsg(e), severity: 'error' });
    }
  }, [pagesPage, env]);
  ...
  const handleDeletePage = async (id: number) => {
    ...
    try { await deleteMagentoCmsPage(id, env); setToast({ message: 'CMS page deleted', severity: 'success' }); loadPages(); } catch (e: unknown) { setToast({ message: getErrMsg(e), severity: 'error' }); }
  };

  const handleDeleteBlock = async (id: number) => {
    if (!confirm(`Delete CMS block #${id}?`)) return;
    try { await deleteMagentoCmsBlock(id, env); setToast({ message: 'CMS block deleted', severity: 'success' }); loadBlocks(); } catch (e: unknown) { setToast({ message: getErrMsg(e), severity: 'error' }); }
  };
  ...
  const handleHomeUpload = async () => {
    ...
    for (const file of homeUploadFiles) {
      ...
      try {
        ...
        setToast({ message: `Upload failed: ${getErrMsg(e)}`, severity: 'error' });
      }
    }
  ...
  if (loading && !pages.length && !blocks.length) return (
    <Box>
      <Skeleton variant="text" width={200} height={44} sx={{ mb: 0.5 }} />
      <Skeleton variant="text" width={320} height={24} sx={{ mb: 2 }} />
      <Skeleton variant="rounded" height={48} sx={{ mb: 1 }} />
      {[...Array(5)].map((_, i) => <Skeleton key={i} variant="rounded" height={52} sx={{ mb: 0.5 }} />)}
    </Box>
  );
