...
import { Box, Typography, Grid, Card, CardContent, Switch, FormControlLabel, TextField, Button, Divider, Alert, Tabs, Tab, List, ListItem, ListItemText, InputAdornment, IconButton, Chip, Select, MenuItem, FormControl, InputLabel, Avatar, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Snackbar } from '@mui/material';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) setLoadError(getErrMsg(err));
      }
    };
    ...
    const loadEmail = async () => {
      try {
        ...
        // non-critical: email settings load failed silently
      }
    };
  ...
  useEffect(() => {
    ...
    const loadSubs = async () => {
      try {
        ...
        // non-critical: push subscriptions load failed silently
      }
    };
  ...
  const saveAll = useCallback(async () => {
    ...
    try {
      ...
      showSnack(getErrMsg(err) || 'Failed to save settings', 'error');
    } finally {
      setSaving(false);
    }
  ...
  const handleUnsubscribeDevice = async (subscriptionId: number) => {
    try {
      ...
      showSnack(getErrMsg(err) || 'Failed to unsubscribe device', 'error');
    }
  };
  ...
  const handleEmailSave = async () => {
    ...
    try {
      ...
      setEmailTestResult({ success: false, message: getErrMsg(err) || 'Failed to save email settings' });
    } finally {
  ...
  const handleEmailTest = async () => {
    ...
    try {
      ...
      setEmailTestResult({ success: false, message: getErrMsg(err) || 'Failed to send test email' });
    } finally {
  ...
  const loadEmailLogs = useCallback(async () => {
    ...
    try {
      ...
      showSnack(getErrMsg(err) || 'Failed to load email logs', 'error');
    } finally {
  ...
  const handleClearEmailLogs = async () => {
    try {
      ...
      showSnack(getErrMsg(err) || 'Failed to clear email logs', 'error');
    }
  };
