import {
  ...
  TableContainer, TableHead, TableRow, Switch, Paper, Chip,
...
const ROLE_LABELS: Record<Role, string> = {
  admin: 'Admin', editor: 'Editor', moderator: 'Moderator',
  viewer: 'Viewer', marketing: 'Marketing',
};

// MUI Chip color prop is a strict union — map each role to a valid value
type ChipColor = 'error' | 'primary' | 'warning' | 'default' | 'secondary';
const ROLE_COLORS: Record<Role, ChipColor> = {
  admin:     'error',
  editor:    'primary',
  moderator: 'warning',
  viewer:    'default',
...
export default function PermissionsPage() {
  ...
  const fetcher = useCallback(
    (_signal?: AbortSignal) => fetchAllRolePermissions() as Promise<Record<string, RolePermissions>>,
    [],
  );
  const { data: permissions, loading } = usePolling<Record<string, RolePermissions>>(fetcher, 0);

  const handleToggle = async (role: string, permission: string, value: boolean) => {
    setSaving(true);
    try {
      await updateRolePermission(role, permission, value);
      setSnackbar({ open: true, message: `Updated ${ROLE_LABELS[role as Role] ?? role} permission`, severity: 'success' });
    } catch (e: unknown) {
      setSnackbar({ open: true, message: extractMessage(e) || 'Failed to update', severity: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading || !permissions) {
    return (
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 2 }}>
        ...
        <Typography color="text.secondary">Loading permissions matrix...</Typography>
      </Box>
    );
  }
                  ...
                  <TableRow key={`group-${group.label}`}>
                    <TableCell
                      ...
                      sx={{
                        ...
                        color: 'primary.main', letterSpacing: '0.05em', textTransform: 'uppercase',
                      }}
                    >
                      {group.label}
                    </TableCell>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 2 }}>
            <Box>
              ...
              <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                Full access to all pages, tasks, notes, and user management.
              </Typography>
            </Box>
            <Box>
              <Chip label="Editor" size="small" color="primary" sx={{ mb: 1, fontWeight: 700 }} />
              <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                Can create tasks, edit own tasks, add/edit/delete own notes. Limited page access.
              </Typography>
            </Box>
            <Box>
              <Chip label="Moderator" size="small" color="warning" sx={{ mb: 1, fontWeight: 700 }} />
              <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                Like Editor but can edit any task, delete any note, and access Settings/Cache/Process Explorer.
              </Typography>
            </Box>
            <Box>
              <Chip label="Viewer" size="small" sx={{ mb: 1, fontWeight: 700 }} />
              <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                Read-only access to most pages. Can create tasks and manage own notes.
              </Typography>
            </Box>
            <Box>
              <Chip label="Marketing" size="small" color="secondary" sx={{ mb: 1, fontWeight: 700 }} />
              <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                Can manage push notifications, view subscriber analytics, and manage segments.
              </Typography>
            </Box>
          </Box>
        </CardContent>
