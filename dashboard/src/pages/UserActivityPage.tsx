import {
  ...
  Collapse, TablePagination, Skeleton, CircularProgress
} from '@mui/material';
...
import { fetchUserActivity, fetchBashHistory } from '../api/monitor';
import type { UserData, BashHistoryEntry } from '../api/monitor';
...
function UserRow({ user, expanded, onToggle }: { user: UserData; expanded: boolean; onToggle: () => void }) {
  ...
  const loadBashHistory = useCallback(() => {
    ...
    setLoadingBash(true);
    fetchBashHistory(user.username, 30)
      .then((data) => { setBashHistory(data.history || []); })
      .catch(() => {})
      .finally(() => setLoadingBash(false));
  }, [user.username, bashHistory.length]);

  // load on expand
  if (expanded && !loadingBash && bashHistory.length === 0) {
    loadBashHistory();
  }
  ...
  return (
    <>
      <TableRow
        onClick={onToggle}
        sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'rgba(255,255,255,0.02)' }, borderBottom: '1px solid rgba(255,255,255,0.04)' }}
      >
...
export default function UserActivityPage() {
  ...
  // stale-while-revalidate: no flicker on 30s background polls
  const fetcher = useCallback(async (): Promise<UserActivityData> => {
    const d = await fetchUserActivity();
    return d as UserActivityData;
  }, []);

  const { data, loading, refreshing, refetch } = usePolling<UserActivityData>(fetcher, 30_000);
    ...
    return (
      <Box>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          {[...Array(4)].map((_, i) => <Grid key={i} size={{ xs: 6, md: 3 }}><Skeleton variant="rounded" height={80} /></Grid>)}
        </Grid>
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
          ...
          <IconButton size="small" onClick={refetch} disabled={refreshing} sx={{ ml: 'auto' }}>
      ...
      <TableContainer component={Paper} sx={{ bgcolor: 'transparent', border: '1px solid rgba(255,255,255,0.06)', flex: 1, overflow: 'auto' }}>
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              {['User','Role','Sessions','SSH','Processes','Disk','Last Login','Status',''].map((h) => (
                <TableCell key={h} sx={{ fontWeight: 700, color: 'text.primary', fontSize: '0.7rem', bgcolor: 'rgba(255,255,255,0.03)' }}>{h}</TableCell>
              ))}
            </TableRow>
          </TableHead>
          <TableBody>
