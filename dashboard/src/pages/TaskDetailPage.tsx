import { Box, Typography, Card, CardContent, Button, Chip, TextField, IconButton, Tooltip, Divider, Avatar, CircularProgress, Skeleton, FormControl, InputLabel, Select, MenuItem, Paper, Menu, MenuItem as MuiMenuItem, List, ListItemButton, ListItemAvatar, ListItemText, Dialog, DialogTitle, DialogContent, DialogActions, Link, Snackbar, Alert } from '@mui/material';
import { ArrowBack, Delete, Send, CheckCircle, Edit, PushPin, Reply, MoreVert, Save, Close, AddPhotoAlternate, Link as LinkIcon, LinkOff } from '@mui/icons-material';
import { useState, useEffect, useRef, useCallback } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { fetchTasks, fetchTask, updateTask, fetchTaskNotes, addNote, deleteNote, editNote, pinNote, fetchTaskActivity, fetchScreenshots, deleteScreenshot, forwardNote, setNoteStatus, uploadScreenshot, getTaskLinks, linkTask, unlinkTask, type Task, type TaskNote, type TaskActivity, type TaskScreenshot, type TaskLink, getTaskStatusColor, NOTE_CATEGORIES, TASK_CATEGORIES } from '../api/tasks';
...
export default function TaskDetailPage() {
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      loadData();
    } catch (e: unknown) { showSnack(getErrMsg(e), 'error'); }
  ...
  const handleScreenshotUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      loadData();
      showSnack('Screenshot uploaded', 'success');
    } catch (err: unknown) {
      showSnack(getErrMsg(err), 'error');
    } finally {
  ...
  const handleDeleteScreenshot = async (screenshotId: number) => {
    try {
      ...
      showSnack('Screenshot removed', 'success');
    } catch (err: unknown) {
      showSnack(getErrMsg(err), 'error');
    }
  };
  ...
  const handleDeleteNote = async (noteId: number) => {
    try { await deleteNote(noteId); loadData(); showSnack('Note deleted', 'success'); } catch (e: unknown) { showSnack(getErrMsg(e), 'error'); }
  };
  ...
  const handleLinkTask = async () => {
    ...
    try {
      ...
      setLinkTargetTaskId('');
      loadData();
      showSnack('Tasks linked', 'success');
    } catch (err: unknown) {
      showSnack(getErrMsg(err), 'error');
    }
  };
  ...
  const handleUnlinkTask = async (linkId: number) => {
    try {
      ...
      loadData();
      showSnack('Link removed', 'success');
    } catch (err: unknown) {
      showSnack(getErrMsg(err), 'error');
    }
  };
  ...
  const linkTypeConfig: Record<string, { label: string; color: ChipColor; icon: string }> = {
                  ...
                  {taskLinks.map(link => {
                    ...
                    return (
                      <Box key={link.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, borderRadius: 1, backgroundColor: 'background.default', border: '1px solid', borderColor: 'divider' }}>
                        <Chip label={`${config.icon} ${config.label}`} size="small" color={config.color} sx={{ height: 20, fontSize: '0.6rem' }} />
                        <Link 
                          component="button"
              ...
              <Box key={note.id} sx={{ ml: note.parent_id ? 4 : 0, borderLeft: note.parent_id ? '2px solid rgba(255,255,255,0.05)' : 'none', pl: note.parent_id ? 2 : 0 }}>
                <Paper sx={{ p: 2, backgroundColor: note.is_pinned ? 'rgba(59,130,246,0.06)' : 'rgba(255,255,255,0.02)', border: note.is_pinned ? '1px solid rgba(59,130,246,0.2)' : '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                    ...
                    <Chip label={`${cat.icon} ${cat.label}`} size="small" color={cat.color} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 600 }} />
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                    <FormControl size="small" sx={{ width: 130 }}>
                      ...
                      <Select
                        value={noteCategory}
                        label="Category"
                        onChange={(e) => setNoteCategory(e.target.value as 'tuning' | 'fix' | 'implementation' | 'question' | 'general')}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                <FormControl fullWidth size="small">
                  ...
                  <Select value={editData.priority} label="Priority" onChange={(e) => setEditData({ ...editData, priority: e.target.value as 'low' | 'medium' | 'high' })}>
                    <MenuItem value="low">Low</MenuItem><MenuItem value="medium">Medium</MenuItem><MenuItem value="high">High</MenuItem>
                  </Select>
                </FormControl>
                <FormControl fullWidth size="small">
                  <InputLabel>Status</InputLabel>
                  <Select value={editData.status} label="Status" onChange={(e) => setEditData({ ...editData, status: e.target.value as 'pending' | 'in-progress' | 'completed' | 'cancelled' })}>
