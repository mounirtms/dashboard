import { Grid, Box, Typography, Card, CardContent, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Button, Divider, Alert, Skeleton, useTheme } from '@mui/material';
...
export default function SystemOverviewPage() {
  ...
  type InsightSeverity = 'success' | 'info' | 'warning' | 'error';
  const insights: { type: InsightSeverity; text: string }[] = [];
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'grid', gap: 1 }}>
                  {insights.map((insight, i) => (
                    <Alert key={i} severity={insight.type} sx={{ '& .MuiAlert-message': { fontSize: '0.75rem' }, py: 0 }}>
                      {insight.text}
                    </Alert>
                  ))}
                </Box>
