...
import {
  ...
  Tooltip, Alert, LinearProgress, Divider, Accordion, AccordionSummary,
  AccordionDetails, Skeleton,
} from '@mui/material';
import {
...
async function fetchBackups(): Promise<BackupData> {
...
export default function BackupsPage() {
  ...
  const fetcher = useCallback(() => fetchBackups(), []);
  // Backups don't change frequently — poll every 5 minutes
  const { data, loading, refreshing, error, refetch } = usePolling<BackupData>(fetcher, 300_000);
          ...
          <TableBody>
            {files.map((f) => (
              <TableRow key={f.file + f.name} hover>
                ...
                <TableCell>
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>{f.size_human}</Typography>
                </TableCell>
                <TableCell>
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>{f.date}</Typography>
                </TableCell>
  ...
  const renderGroup = (group: BackupGroup, index: number) => {
    ...
    const totalFiles   = (group.accounts?.length || 0) + (group.databases?.length || 0)
                        + (group.system?.length || 0)  + (group.configs?.length || 0);
  ...
  if (loading) {
    return (
      <Box>
        ...
        {[...Array(3)].map((_, i) => <Skeleton key={i} height={80} sx={{ mb: 2 }} />)}
      </Box>
    );
  }
    ...
    <Box>
      ...
      <Box sx={{ mt: 3, p: 2, background: 'rgba(0,0,0,0.2)', borderRadius: 1 }}>
        <Typography variant="caption" sx={{ color: 'text.disabled', display: 'block' }}>
          Backups generated by WHM (pkgacct), MariaDB 10.6 mysqldump on port 3307, and streamlined backup script (server configs).
        </Typography>
        <Typography variant="caption" sx={{ color: 'text.disabled', display: 'block' }}>
