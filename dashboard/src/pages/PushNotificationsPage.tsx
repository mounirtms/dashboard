...
export default function PushNotificationsPage() {
  ...
  const loadAlertLog = async () => {
    ...
    try {
      const response = await apiClient.get('/api/monitor.php?action=notification_log');
      const data = response.data;
      if (data.success && data.logs) {
        setAlertLog(data.logs);
      }
    } catch (err: unknown) {
      console.error('Failed to load alert log:', getErrMsg(err));
    } finally {
  ...
  const handleIconUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Icon upload failed: ' + (getErrMsg(err) || 'Unknown error'), severity: 'error' });
      setIconFile(null);
      setIconPreview('');
    } finally {
      setUploading(false);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Image upload failed: ' + (getErrMsg(err) || 'Unknown error'), severity: 'error' });
      setImageFile(null);
      setImagePreview('');
    } finally {
      setUploading(false);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Error: ' + getErrMsg(e), severity: 'error' });
    } finally {
      setSending(false);
    }
  };
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Re-sync failed: ' + getErrMsg(e), severity: 'error' });
    } finally {
      setSyncing(false);
    }
  };
  ...
  if (loading && !stats) return <Skeleton variant="rectangular" height={400} sx={{ borderRadius: 2, mx: 2, my: 3 }} />;
