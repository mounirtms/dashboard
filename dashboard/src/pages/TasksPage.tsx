import {
  ...
  Checkbox, Skeleton, LinearProgress
} from '@mui/material';
import { DataGrid, GridColDef, GridRenderCellParams } from '@mui/x-data-grid';
import {
  Add, Edit, Delete, CheckCircle, Refresh, FilterList, MoreVert,
  Notes, Download, Person, LinkOff, Schedule
} from '@mui/icons-material';
import { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  fetchTasks, createTask, updateTask, deleteTask, fetchTaskStats,
  fetchTaskNotesCount, bulkUpdate,
  type Task, type TaskStats,
...
export default function TasksPage() {
  ...
  const [refreshing, setRefreshing]     = useState(false);  // ← background poll indicator
  ...
  const [formData, setFormData]         = useState({
    ...
    assigned_to: '', due_date: '', category: 'general',
  });
  ...
  // ---- data load (stale-while-revalidate pattern without usePolling because of complex filter deps) ----
  ...
  useEffect(() => {
    loadData(hasFetched.current);   // first fetch: full load; subsequent: background
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterStatus, filterPriority, filterCategory, filterAssignee, myTasksOnly, showOverdueOnly, search, pagination.page, pagination.perPage]);
  ...
  const handleBulkAction = async (action: string, value?: string) => {
    ...
    try {
      ...
      if (action === 'priority') fields.priority    = value;
      if (action === 'assigned_to') fields.assigned_to = value;
  ...
  const dueDateUrgency = (task: Task) => {
    ...
    const diffDays = Math.ceil((new Date(task.due_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    if (diffDays < 0 && task.status !== 'completed' && task.status !== 'cancelled')
      return { color: 'error' as const, label: `${Math.abs(diffDays)}d overdue` };
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'priority', headerName: 'Priority', width: 100, renderCell: (p: GridRenderCellParams) =>
      <Chip label={p.value.toUpperCase()} size="small" color={getTaskPriorityColor(p.value)} sx={{ fontWeight: 700, fontSize: '0.6rem' }} />
    },
    { field: 'status', headerName: 'Status', width: 120, renderCell: (p: GridRenderCellParams) =>
      <StatusBadge label={p.value.toUpperCase().replace('-', ' ')} color={getTaskStatusColor(p.value)} />
    },
    { field: 'assigned_to', headerName: 'Assigned', width: 120, renderCell: (p: GridRenderCellParams) => (
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
        {p.value && <Avatar sx={{ width: 20, height: 20, fontSize: '0.6rem', bgcolor: 'primary.main' }}>{String(p.value).charAt(0).toUpperCase()}</Avatar>}
    ...
    { field: 'category', headerName: 'Category', width: 100, renderCell: (p: GridRenderCellParams) =>
      <Typography variant="caption" sx={{ textTransform: 'capitalize' }}>{p.value}</Typography>
    },
  ...
  if (loading && tasks.length === 0) {
    return (
      <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Skeleton variant="text" width={160} height={44} sx={{ mb: 0.5 }} />
      ...
      <Box sx={{ mb: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button size="small" variant="outlined" startIcon={<Refresh />} onClick={() => loadData()} disabled={refreshing}>Sync</Button>
            {permissions?.can_create_tasks && <Button size="small" variant="contained" startIcon={<Add />} onClick={openCreate}>New Task</Button>}
          </Box>
        </Box>

        {/* Refreshing indicator — replaces full LoadingState flicker */}
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <Tooltip title="Clear Filters">
              <IconButton size="small" onClick={() => {
                ...
                setMyTasksOnly(false); setShowOverdueOnly(false);
              }}>
            ...
            <Menu anchorEl={moreAnchor} open={Boolean(moreAnchor)} onClose={() => setMoreAnchor(null)}>
              ...
              <MenuItem onClick={() => { setMoreAnchor(null); exportCSV(); }}>
                <Download sx={{ fontSize: 16, mr: 1 }} />Export CSV
              </MenuItem>
            </Menu>
          </Box>
        </Card>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <FormControl fullWidth size="small">
                ...
                <Select value={formData.priority} label="Priority" onChange={(e) => setFormData({ ...formData, priority: e.target.value as typeof formData.priority })}>
                  <MenuItem value="low">Low</MenuItem><MenuItem value="medium">Medium</MenuItem><MenuItem value="high">High</MenuItem>
                </Select>
              </FormControl>
              <FormControl fullWidth size="small">
                <InputLabel>Status</InputLabel>
                <Select value={formData.status} label="Status" onChange={(e) => setFormData({ ...formData, status: e.target.value as typeof formData.status })}>
                  <MenuItem value="pending">Pending</MenuItem><MenuItem value="in-progress">In Progress</MenuItem>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <FormControl fullWidth size="small">
                ...
                <Select value={formData.assigned_to} label="Assign To" onChange={(e) => setFormData({ ...formData, assigned_to: e.target.value })}>
