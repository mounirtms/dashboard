...
class WebpushrAlertHelper {
    ...
    public static function sendAlert($severity, $title, $message, $alertKey = null) {
        ...
        if ($alertKey) {
            ...
            // Guard: $ts must be a scalar int/float; arrays (from corrupt state) are purged
            foreach ($state as $k => $ts) {
                if (!is_int($ts) && !is_float($ts) || ($now - (int)$ts) > 3600) {
                    unset($state[$k]);
                }
            }
        ...
        if ($result['success']) {
            ...
            self::log('FAILED', "$title - $shortMessage (" . ($result['error'] ?? 'unknown error') . ")");
        }
