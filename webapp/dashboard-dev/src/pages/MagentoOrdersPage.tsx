...
export default function MagentoOrdersPage() {
  ...
  // Non-fatal error — show inline alert above the grid instead of replacing the whole page

  return (
    <Box>
      {error && (
        <Alert severity="warning" sx={{ mb: 2 }} action={<Button color="inherit" size="small" onClick={loadOrders}>Retry</Button>}>
          {error}
        </Alert>
      )}
      ...
      {/* Multi-Year KPI Banner — Real DB data */}
      {env === 'prod' && (
          ...
          <Box sx={{ display: 'flex', gap: 2.5, flexWrap: 'wrap', alignItems: 'center' }}>
            {[
              ...
              <Box key={yr} sx={{ borderLeft: `3px solid ${color}`, pl: 1.5 }}>
                ...
                <Typography variant="h5" sx={{ fontWeight: 900, color, lineHeight: 1 }}>{orders}</Typography>
                <Typography variant="caption" sx={{ color: 'text.disabled' }}>AOV {aov} DZD</Typography>
              </Box>
            ))}
      ...
      <DataGrid
        rows={orders ?? []} columns={columns} rowCount={totalCount ?? 0} loading={loading}
